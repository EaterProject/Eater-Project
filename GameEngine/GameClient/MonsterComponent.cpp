...
bool MonsterComponent::GetStopPoint(int Index)
{
	...
	if (mTransform->Position.x > (SearchPoint[Index].x - 0.5f) && mTransform->Position.x < (SearchPoint[Index].x + 0.5f) &&
		mTransform->Position.z >(SearchPoint[Index].z - 0.5f)  && mTransform->Position.z < (SearchPoint[Index].z + 0.5f))
	{
		return true;
