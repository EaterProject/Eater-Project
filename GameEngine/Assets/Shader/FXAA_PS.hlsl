...
{
    float4 gFrame;//= float4(1.0f / 1280, 1.0f / 1080, 0, 0);
};
...
{
    return gOriginMap.SampleLevel(gSamClampAnisotropic, pos.xy, 0.0);
}

float4 FxaaTexGrad(float2 pos, float2 grad)
{
    return gOriginMap.SampleGrad(gSamClampAnisotropic, pos.xy, grad, grad);
}

float4 FxaaTexOff(float2 pos, int2 off)
{
    return gOriginMap.SampleLevel(gSamClampAnisotropic, pos.xy, 0.0, off.xy);
}
...
float4 FXAA_PS(ScreenPixelIn pin) : SV_TARGET
{
    return float4(FxaaPixelShader(pin.Tex), 1.0f);
}
