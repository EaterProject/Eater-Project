...
void Blur_Pass::Start(int width, int height)
{
	...
	m_BlurSRV = m_Blur_RT->GetSRV()->Get();
	m_BlurUAV = m_Blur_RT->GetUAV()->Get();
...
void Blur_Pass::RenderUpdate(RenderTexture* input, UINT count)
{
	...
	for (UINT i = 0; i < count; i++)
	{
		// HORIZONTAL blur pass.
		m_BlurHorizon_CS->SetShaderResourceView<gInputMap>(input->GetSRV()->Get());
		m_BlurHorizon_CS->SetUnorderedAccessView<gOutputUAV>(m_BlurUAV);
		...
		// VERTICAL blur pass.
		m_BlurVertical_CS->SetShaderResourceView<gInputMap>(m_BlurSRV);
		m_BlurVertical_CS->SetUnorderedAccessView<gOutputUAV>(input->GetUAV()->Get());
...
void Blur_Pass::ScreenBlur(UINT count)
{
	g_Context->OMSetRenderTargets(0, nullptr, nullptr);

	ID3D11ShaderResourceView* inputSRV = g_BindingRenderTarget->GetSRV()->Get();
	ID3D11UnorderedAccessView* inputUAV = g_BindingRenderTarget->GetUAV()->Get();

	for (UINT i = 0; i < count; i++)
	{
		// HORIZONTAL blur pass.
		m_BlurHorizon_CS->SetShaderResourceView<gInputMap>(inputSRV);
		m_BlurHorizon_CS->SetUnorderedAccessView<gOutputUAV>(m_BlurUAV);
		...
		// VERTICAL blur pass.
		m_BlurVertical_CS->SetShaderResourceView<gInputMap>(m_BlurSRV);
		m_BlurVertical_CS->SetUnorderedAccessView<gOutputUAV>(inputUAV);
