...
void Culling_Pass::Create(int width, int height)
{
	m_Width = 512.0f;
	m_Height = 256.0f;
...
void Culling_Pass::Start(int width, int height)
{
	// Shader 설정..
...
void Culling_Pass::OnResize(int width, int height)
{
...
void Culling_Pass::InstanceResize(size_t& renderMaxCount, size_t& unRenderMaxCount)
{
	CullingBufferCreate();
}

void Culling_Pass::Release()
{

}

void Culling_Pass::RenderOccluders()
{
...
void Culling_Pass::DrawStateUpdate()
{
	if (CullingRenderMeshList.empty() || g_GlobalData->OccluderList.empty()) return;
...
void Culling_Pass::FrustumCulling()
{
	...
	for (int i = 0; i < CullingRenderMeshList.size(); i++)
	{
		...
		boundSphere = m_RenderData->m_Mesh->m_MeshProperty->BoundSphere;
...
void Culling_Pass::MipMapResourceRelease()
{
...
void Culling_Pass::MipMapResourceCreate()
{
...
void Culling_Pass::MipMapCommandListReserve(const D3D11_TEXTURE2D_DESC* hizDesc)
{
...
void Culling_Pass::CullingBufferCreate()
{
